...
int main(){
    ...
    //PHASE 1// prompt user to enter name and choose difficulty level (house size), send them to the first merchant    
    string username="";
    int house_size=0;
    ...
    cin >> username;
    game.MENU.MAP.AREAS.STATS.setUsername(username);
    getchar();
    cout << "Tom Nook 🦝: 'Alright " << game.MENU.MAP.AREAS.STATS.getUsername() << ", welcome to your potential new home.'" << endl;
    getchar();
    ...
    game.MENU.MAP.AREAS.STATS.setDebt(house_size);
    getchar();
    cout << "Tom Nook 🦝: 'Okay then... your total comes out to be " << game.MENU.MAP.AREAS.STATS.getDebt() << " bells.'" << endl;
    getchar();
    ...
    getchar();
    cout << "Isabelle 🐶: 'I- yes. Well, I was thinking " << game.MENU.MAP.AREAS.STATS.getUsername() << " could spend a day on the island and collect resources to pay you back.'" << endl;
    getchar();
    cout << "Tom Nook 🦝: 'Oh... well that's not a half bad idea. But I don't care. Just get me the money, " << game.MENU.MAP.AREAS.STATS.getUsername() << ". Figure it out.'" << endl;
    getchar();
    cout << "..." << endl;
    getchar();
    ...
    getchar();
    cout << "Isabelle 🐶: 'To get into the Beach, you need to have at least 3 " << game.MENU.MAP.AREAS.STATS.getFruitNamePlural() << ", just so you don't get hungry on your adventure.'" << endl;
    getchar();
    ...
    game.MENU.MAP.AREAS.Nooks_Cranny();//merchant

    getchar();
    cout << "Isabelle 🐶: 'I think you're all ready to go! Stay safe and make sure to eat every once and a while! You can find " << game.MENU.MAP.AREAS.STATS.getFruitNamePlural() << " on the trees!" << endl;
    getchar();
    ...
    do{
        game.MENU.MAP.AREAS.STATS.DisplayStats();
        game.MENU.MAP.displayMap();
        game.MENU.checkMenuType();
    }
    while(game.MENU.MAP.AREAS.STATS.getTomPatience()>0 && game.MENU.MAP.isDungeonExit(game.MENU.MAP.getPlayerRow(),game.MENU.MAP.getPlayerCol())==false && game.MENU.getEndValue()==false && game.MENU.MAP.AREAS.STATS.getFullness()>0); 
   ...
   game.MENU.MAP.AREAS.STATS.SaveStats();//save number of items before they all get sold

    if (game.MENU.MAP.AREAS.STATS.getFullness()<=0)// run out of fullness
    {
        ...
        cout << "" << endl;
        game.MENU.MAP.AREAS.STATS.displayScoreboard();
    }

    else if (game.MENU.MAP.AREAS.STATS.getTomPatience()==0)// tom patience runs out
    {
        cout << "Tom's Patience has reached 0." << endl;
        getchar();
        cout << "Tom Nook 🦝: '" << game.MENU.MAP.AREAS.STATS.getUsername() << "!!! I have HAD it. Your house is sold to someone else. Anyone else. You don't get the house.'" << endl;
        getchar();
    ...
    else if(game.MENU.MAP.isDungeonExit(game.MENU.MAP.getPlayerRow(),game.MENU.MAP.getPlayerCol())==true)// they reach the end
    {
        game.MENU.MAP.AREAS.Nooks_CrannyEnd();// go and sell all their stuff

        cout << "Tom Nook 🦝: 'Well well well.'" << endl;
        getchar();
        cout << "Tom Nook 🦝: 'Your debt is " << game.MENU.MAP.AREAS.STATS.getDebt() << " bells and you made " << game.MENU.MAP.AREAS.STATS.getNumBells() << " bells.'" << endl;
        getchar();

        if(game.MENU.MAP.AREAS.STATS.getNumBells() >=game.MENU.MAP.AREAS.STATS.getDebt())// if the total money is greater or equal to the debt (they win!)
        {
    ...
    else if(game.MENU.getEndValue()==true)//the player quits
    {
        cout << "💐Thanks for joining! See you next time!💐" << endl;
        cout << "" << endl;
        game.MENU.MAP.AREAS.STATS.displayScoreboard();
